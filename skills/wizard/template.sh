...
# On a headless session (no display, not WSL/macOS) it prints the URL to
# visit from another device instead of attempting to open a browser.
open_url() {
  local url="$1"
  if [[ -z "${DISPLAY:-}" && -z "${WAYLAND_DISPLAY:-}" && "$(uname -s)" != "Darwin" ]] \
     && ! command -v wslview >/dev/null 2>&1 && ! command -v explorer.exe >/dev/null 2>&1; then
    printf '  %s↗ visit from any device:%s %s\n' "$GREEN" "$RESET" "$url"
    return 0
  fi
